...
/* minor number for the misc device */
#define SIM_MINOR 254
...
#define MXC_SIM_CLK 4000000 	/* 4MHz clock */
...
/* Used to active the SIM bus */
extern void gpio_sim_active(void);
extern void gpio_sim_inactive(void);
...
static void sim_start(sim_t *sim)
{
	...
	/* ~ 4 MHz */
	clk_rate = clk_get_rate(sim_clk);
	clk_div = clk_rate / MXC_SIM_CLK;
	if (clk_rate % MXC_SIM_CLK)
		clk_div++;
...
static int sim_open(struct inode *inode, struct file *file)
{
	...
	sim_t *sim = kcalloc(1, sizeof(sim_t), GFP_KERNEL);

	if (sim == 0) {
		errval = -SIM_E_MALLOC;
		printk(KERN_ERR "Can't get the MEMORY\n");
		goto sim_open_err_out;
	};

	errval = request_irq(MXC_INT_SIM_IPB, sim_irq_handler,
			     0, "mxc_sim_ipb", sim);
	errval |= request_irq(MXC_INT_SIM_DAT, sim_irq_handler,
			      0, "mxc_sim_dat", sim);

	if (errval) {
		free_irq(MXC_INT_SIM_IPB, sim);
		free_irq(MXC_INT_SIM_DAT, sim);
		kfree(sim);
		errval = -SIM_E_IRQ;
	...
	if (!request_mem_region(SIM_BASE_ADDR, 0x1000, "mxc_sim")) {
		printk(KERN_ERR "request_mem_region failed\n");
		errval = -ENOMEM;
		goto sim_open_err_out;
	}
	sim->ioaddr = (void *) ioremap((uint32_t) SIM_BASE_ADDR, 0x1000);;
	sim_start(sim);
...
static int sim_release(struct inode *inode, struct file *file)
{
	...
	release_mem_region(SIM_BASE_ADDR, 0x1000);
...
static int sim_init(void)
{
	...
	/* enable sim_clk and sim_serial_clk */
	sim_clk = clk_get(NULL, "sim_clk");
	if (IS_ERR(sim_clk)) {
		ret = PTR_ERR(sim_clk);
	...
	clk_enable(sim_clk);
	pr_debug("sim clock:%lu\n", clk_get_rate(sim_clk));

	misc_register(&sim_dev);

	printk(KERN_INFO "MXC SIM Interface driver\n");
...
{
	/* disable sim_clk and sim_serial_clk */
	clk_disable(sim_clk);
	clk_put(sim_clk);
