...
{
    	return I2C_ReadByte(TX_TPI_ADDR, Offset);
}
...
{
    	I2C_WriteByte(TX_TPI_ADDR, Offset, Data);
}
...
{
    	I2C_ReadBlock(TX_TPI_ADDR, TPI_Offset, NBytes, pData);
}
...
{
    	I2C_WriteBlock(TX_TPI_ADDR, TPI_Offset, NBytes, pData);
}
...
uint8_t sii_I2CReadByte (uint8_t SlaveAddr, uint8_t RegAddr)
{
	...
	if (!TPImode)
		regVal = I2C_ReadByte(SlaveAddr, RegAddr);
	else
	{
...
{
	if (!TPImode)
		I2C_WriteByte(SlaveAddr, RegAddr, Data);
	else
	{
		if( SlaveAddr == PAGE_0_0X72 )
...
uint8_t ParseEDID (uint8_t *pEdid, uint8_t *numExt)
{
	...
	TX_DEBUG_PRINT(("\n"));
	TX_DEBUG_PRINT(("EDID DATA (Segment = 0 Block = 0 Offset = %d):\n", (int) EDID_BLOCK_0_OFFSET));
...
uint8_t Parse861Extensions (uint8_t NumOfExtensions)
{
    ...
    do
    {
        ...
        TX_DEBUG_PRINT(("\n"));
        TX_DEBUG_PRINT(("EDID DATA (Segment = %d Block = %d Offset = %d):\n", (int) Segment, (int) Block, (int) Offset));
        for (j=0, i=0; j<128; j++)
        {
            k = g_CommData[j];
            TX_DEBUG_PRINT(("%2.2X ", (int) k));
...
{
    	I2C_ReadBlock(HDCP_SLAVE_ADDR, TPI_Offset, NBytes, pData);
}
...
// 4K x 2K Modes:
    ...
    //        1   2  SubM   PixClk  V/H Position       VFreq   H      V      H    V       H    V    Ratio    Adj  H   V  Top  Dly HSync VSync Offset Repl  Space1 Space2 Blank1 Blank2 Blank3  3D
    //===================================================================================================================================================================================================================================
    {{        1,  0, NSM}, 29700, {ProgrVNegHNeg,     3000,{4400, 2250}}, {384, 82}, {3840,2160}, R_16,    {0,  88, 10, 72, 296, 176,  8,    0},   0,     {0,     0,     0,     0,    0},    NO_3D_SUPPORT}, // 43 - 4k x 2k 29.97/30Hz (297.000 MHz)
    {{        2,  0, NSM}, 29700, {ProgrVNegHNeg,     2500,{5280, 2250}}, {384, 82}, {3840,2160}, R_16,    {0,  88, 10, 72, 296,1056,  8,    0},   0,     {0,     0,     0,     0,    0},    NO_3D_SUPPORT}, // 44 - 4k x 2k 25Hz
    {{        3,  0, NSM}, 29700, {ProgrVNegHNeg,     2400,{5500, 2250}}, {384, 82}, {3840,2160}, R_16,    {0,  88, 10, 72, 296,1276,  8,    0},   0,     {0,     0,     0,     0,    0},    NO_3D_SUPPORT}, // 45 - 4k x 2k 24Hz (297.000 MHz)
    {{        4  ,0, NSM}, 29700, {ProgrVNegHNeg,     2400,{6500, 2250}}, {384, 82}, {4096,2160}, R_16,    {0,  88, 10, 72, 296,1020,  8,    0},   0,     {0,     0,     0,     0,    0},    NO_3D_SUPPORT}, // 46 - 4k x 2k 24Hz (SMPTE)
...
//------------------------------------------------------------------------------
const uint8_t AspectRatioTable[] =
{
...
static	bool_t	mscCmdInProgress = 0;	// false when it is okay to send a new command
...
uint8_t OnHdmiCableConnected (void)
{
	...
	TX_DEBUG_PRINT (("[MHL]: ****Reading EDID begin... mscCmdInProgress0 = %d\n", (int)mscCmdInProgress));
...
#else
	sii_I2CWriteByte(PAGE_0_0X72, 0x95, 0x75);
...
{	
        ...
        gpio_direction_output(MX53_PCBA_MHL_WAKE, 0);
        HalTimerWait(T_SRC_WAKE_PULSE_WIDTH_1);	// no adjustmebt for code path in GPIO mode
...
static void Int4Isr( void )
{
	...
	if(0xFF == reg74)
#else
	if(0xFF == reg74 || 0x87 == reg74)	// freescale platform no I2C ack will return 0x87
#endif
	{
		return;
	}
...
{
	// Toggle TX reset pin
	// It will be done in I2C driver proble
...
void	SiiMhlTxGotMhlIntr( uint8_t intr_0, uint8_t intr_1 )
{
	...
	if(MHL_INT_EDID_CHG & intr_1)
	{
		...
		{
			TX_DEBUG_PRINT(("[MHL]: **********************************EDID read fail 1**********************************\n"));
			SiiMhlTxInitialize( true, MONITORING_PERIOD);
			return;
		}
	}
}
