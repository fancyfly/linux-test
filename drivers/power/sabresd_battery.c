...
static void max8903_charger_update_status(struct max8903_data *data)
{
	...
	if (data->charger_online == 0 && data->usb_charger_online == 0) {
		...
		else if (data->ta_in && gpio_get_value(data->pdata->chg) > 0) {
			...
			if (data->charger_online == 0
				...
				volt[i] = max11801_read_adc()-offset_discharger;
					...
					if (data->charger_online == 1)
						volt[i] = max11801_read_adc()
						-offset_charger;
					else if (data->usb_charger_online == 1)
						volt[i] = max11801_read_adc()
						-offset_usb_charger;
					else if (data->charger_online == 1 &&
						data->usb_charger_online == 1)
						volt[i] = max11801_read_adc()
						-offset_charger;
				}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if (gpio_get_value(di->pdata->chg) == 0) {
		...
		} else if (di->ta_in && gpio_get_value(di->pdata->chg) > 0) {
		 ...
		 } else if (di->usb_in && gpio_get_value(di->pdata->chg) > 0) {
			if (!di->pdata->feature_flag) {
				if (di->percent >= 99)
					di->battery_status =
					POWER_SUPPLY_STATUS_FULL;
...
static __devinit int max8903_probe(struct platform_device *pdev)
{
	...
	schedule_delayed_work(&data->work, data->interval);
	if (pdata->dc_valid) {
		ret = request_irq(gpio_to_irq(pdata->dok), max8903_dcin,
	...
	if (pdata->usb_valid) {
		ret = request_irq(gpio_to_irq(pdata->uok), max8903_usbin,
				 IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,
				 "MAX8903 USB IN", data);
		if (ret) {
	...
	if (pdata->flt) {
		ret = request_irq(gpio_to_irq(pdata->flt), max8903_fault,
				 IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,
				 "MAX8903 Fault", data);
		if (ret) {
	...
	if (pdata->chg) {
		...
		ret = request_irq(gpio_to_irq(pdata->chg), max8903_chg,
				 IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,
				 "MAX8903 Status", data);
		if (ret) {
