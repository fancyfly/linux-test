 ...
 *
 * $Id: dhd_linux.c 278830 2011-08-20 00:45:13Z $
 */
...
extern bool ap_fw_loaded;//add by fl
#if defined(SOFTAP)
extern bool ap_cfg_running;
extern bool ap_fw_loaded;
//extern bool ap_fw_loaded=FALSE;//add by fl
#endif
...
#ifdef SOFTAP
	/* skip suspend if apsta enabled. */
	if ( dhd_apsta ) return 0;
#endif
	...
	case WLC_E_IF_ADD:
		...
		if (ret == 0) {
			...
			if (dhd->dhd_state & DHD_ATTACH_STATE_CFG80211 &&
				!(dhd->dhd_state & DHD_ATTACH_STATE_SOFTAP))
...
#if defined(SOFTAP)
		if (ap_fw_loaded && (dhd->dhd_state & DHD_ATTACH_STATE_SOFTAP)) {
...
dhd_open(struct net_device *net)
{
	...
	if (wl_iw_get_onoff() == G_WLAN_SET_OFF) {
		return -1;
	}
...
/* anthony: we still need this due to WEXT priv enabled */
#if !defined(WL_CFG80211) || 1
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	if  ((dhd->pub.busstate == DHD_BUS_DOWN) &&
		...
		printk(KERN_ERR"load firmware from %s\n", fw_path);

		if (!(dhd_bus_download_firmware(dhd->pub.bus, dhd->pub.osh,
		                                fw_path, nv_path))) {
			printk(KERN_ERR"%s: dhdsdio_probe_download failed. firmware = %s nvram = %s\n",
			...
			return -1;
                ...
                if  (strstr(fw_path, "apsta") != NULL) {
                        DHD_ERROR(("GOT APSTA FIRMWARE\n"));
                        ap_fw_loaded = TRUE;
                } else {
                        DHD_ERROR(("GOT STA FIRMWARE\n"));
                        ap_fw_loaded = FALSE;
                }
#endif
	}
	...
	if ((ret = dhd_bus_init(&dhd->pub, FALSE)) != 0) {
		...
		printk(KERN_ERR"%s, dhd_bus_init failed %d\n", __FUNCTION__, ret);
		dhd_os_sdunlock(dhdp);
		return ret;
	}
#if defined(OOB_INTR_ONLY)
...
/* enable dongle roaming event */
	...
	if ((ret = dhd_prot_init(&dhd->pub)) < 0)
	  {
	    printk(KERN_ERR"dhd_prot_init error\r\n");
		return ret;
	  }
#ifdef WRITE_MACADDR
...
dhd_net_attach(dhd_pub_t *dhdp, int ifidx)
{
	...
	strcpy(net->name, "wlan0");//added by fangxiaozhi
...
#if defined(CONFIG_WIRELESS_EXT) && !defined(WL_CFG80211)
#ifdef SOFTAP
			if (ifidx == 0)
...
#if defined(SOFTAP)
	if (ap_fw_loaded) {
		/* Don't do wl_cfg80211_event() when softap on. */
		return (BCME_OK);
	}
#endif
#endif /* defined(CONFIG_WIRELESS_EXT)  */
...
int dhd_os_wake_force_unlock(dhd_pub_t *pub)
{
	...
	if (dhd) {
		spin_lock_irqsave(&dhd->wakelock_spinlock, flags);
		if (dhd->wakelock_counter) {
			printf("wakeloc_counter = %d, set to 0\n", dhd->wakelock_counter);
			dhd->wakelock_counter = 0;
#ifdef CONFIG_HAS_WAKELOCK
			if (!dhd->wakelock_counter)
				wake_unlock(&dhd->wl_wifi);
