...
{
	char di0_clk[] = "ipu1_di0";
	char di1_clk[] = "ipu1_di1";
	char di0_clk_sel[] = "ipu1_di0_sel";
	char di1_clk_sel[] = "ipu1_di1_sel";
	...
	di0_clk[3] += pdata->id;
	di1_clk[3] += pdata->id;
	di0_clk_sel[3] += pdata->id;
	di1_clk_sel[3] += pdata->id;
...
static const struct of_device_id imx_ipuv3_dt_ids[] = {
	{ .compatible = "fsl,imx6q-ipuv3", .data = &imx_ipu_type[IMX6Q_IPU], },
	{ /* sentinel */ }
};
...
static int __devinit ipu_probe(struct platform_device *pdev)
{
	...
	if (pltfm_data->devtype == IPU_V3H) {
		pltfm_data->init = imx6q_src_init_ipu;
		pltfm_data->pg = imx6q_ipu_pg;
	}
	...
	ipu_clk[3] += pltfm_data->id;
	ipu->ipu_clk = devm_clk_get(ipu->dev, ipu_clk);
