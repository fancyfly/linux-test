...
#define CGPR				0x64
#define BM_CGPR_CACHE_LPM_IN_WAIT_BIT		(0x1 << 17)
...
void imx6q_set_cache_lpm_in_wait(bool enable)
{
	if (imx6q_revision() > IMX_CHIP_REVISION_1_1 ||
		...
		if (enable)
			val |= BM_CGPR_CACHE_LPM_IN_WAIT_BIT;
		else
			val &= ~BM_CGPR_CACHE_LPM_IN_WAIT_BIT;
...
int imx6q_set_lpm(enum mxc_cpu_pwr_mode mode)
{
	 ...
	 */
	imx_gpc_single_irq_mask(MX6Q_INT_PARITY_CHECK_ERROR, false);
	val &= ~BM_CLPCR_LPM;
	switch (mode) {
	case WAIT_CLOCKED:
		break;
	...
	case STOP_POWER_OFF:
		...
		break;
	default:
		imx_gpc_single_irq_mask(MX6Q_INT_PARITY_CHECK_ERROR, true);
		return -EINVAL;
	}
 ...
 */
static void imx6q_cscmr1_fixup(u32 *val)
{
	*val ^= CSCMR1_FIXUP;
	return;
}
...
static const char *cko2_sels[]	= { "mmdc_ch0_axi", "mmdc_ch1_axi", "usdhc4", "usdhc1",
				    "gpu2d_axi", "dummy", "ecspi_root", "gpu3d_axi", "usdhc3", "pcie_ref",
				    "dummy", "dummy", "dummy", "vdo_axi", "osc", "gpu2d_core", "gpu3d_core",
				    "usdhc2", "ssi1", "ssi2", "ssi3", "gpu3d_shader", "vpu_axi",
				    "can_root", "dummy", "dummy", "esai", "dummy", "dummy",
				    "dummy", "dummy", "dummy", };
static const char *cko1_cko2_sels[]	= { "cko1", "cko2", };

/* Please add your own lvds src clk, if it is not contained yet. */
static const char *lvds_sels[]	= { "arm", "pll1_sys", "dummy", "dummy", "dummy", "dummy", "pll4_audio_div", "pll5_video_div",
				    "pll5_mlb", "dummy", "pcie_ref", "sata_ref", "usbphy1", "usbphy2", };
...
int __init mx6q_clocks_init(void)
{
	...
	clk[ssi1_sel]         = imx_clk_fixup_mux("ssi1_sel",   base + 0x1c, 10, 2, ssi_sels,          ARRAY_SIZE(ssi_sels),          imx6q_cscmr1_fixup);
	clk[ssi2_sel]         = imx_clk_fixup_mux("ssi2_sel",   base + 0x1c, 12, 2, ssi_sels,          ARRAY_SIZE(ssi_sels),          imx6q_cscmr1_fixup);
	clk[ssi3_sel]         = imx_clk_fixup_mux("ssi3_sel",   base + 0x1c, 14, 2, ssi_sels,          ARRAY_SIZE(ssi_sels),          imx6q_cscmr1_fixup);
	clk[usdhc1_sel]       = imx_clk_fixup_mux("usdhc1_sel", base + 0x1c, 16, 1, usdhc_sels,        ARRAY_SIZE(usdhc_sels),        imx6q_cscmr1_fixup);
	clk[usdhc2_sel]       = imx_clk_fixup_mux("usdhc2_sel", base + 0x1c, 17, 1, usdhc_sels,        ARRAY_SIZE(usdhc_sels),        imx6q_cscmr1_fixup);
	clk[usdhc3_sel]       = imx_clk_fixup_mux("usdhc3_sel", base + 0x1c, 18, 1, usdhc_sels,        ARRAY_SIZE(usdhc_sels),        imx6q_cscmr1_fixup);
	clk[usdhc4_sel]       = imx_clk_fixup_mux("usdhc4_sel", base + 0x1c, 19, 1, usdhc_sels,        ARRAY_SIZE(usdhc_sels),        imx6q_cscmr1_fixup);
	clk[enfc_sel]         = imx_clk_mux("enfc_sel",         base + 0x2c, 16, 2, enfc_sels,         ARRAY_SIZE(enfc_sels));
	clk[emi_sel]          = imx_clk_fixup_mux("emi_sel",      base + 0x1c, 27, 2, emi_sels,        ARRAY_SIZE(emi_sels),          imx6q_cscmr1_fixup);
	clk[emi_slow_sel]     = imx_clk_fixup_mux("emi_slow_sel", base + 0x1c, 29, 2, emi_slow_sels,   ARRAY_SIZE(emi_slow_sels),     imx6q_cscmr1_fixup);
	clk[vdo_axi_sel]      = imx_clk_mux("vdo_axi_sel",      base + 0x18, 11, 1, vdo_axi_sels,      ARRAY_SIZE(vdo_axi_sels));
	clk[vpu_axi_sel]      = imx_clk_mux("vpu_axi_sel",      base + 0x18, 14, 2, vpu_axi_sels,      ARRAY_SIZE(vpu_axi_sels));
	clk[cko1_sel]         = imx_clk_mux("cko1_sel",         base + 0x60, 0,  4, cko1_sels,         ARRAY_SIZE(cko1_sels));
	clk[cko1_cko2_sel]    = imx_clk_mux("cko1_cko2_sel",    base + 0x60, 8,  1, cko1_cko2_sels,    ARRAY_SIZE(cko1_cko2_sels));
	...
	clk[ipg_per]          = imx_clk_fixup_divider("ipg_per",    "ipg",               base + 0x1c, 0,  6, imx6q_cscmr1_fixup);
	...
	clk[emi_podf]         = imx_clk_fixup_divider("emi_podf",   "emi_sel",           base + 0x1c, 20, 3, imx6q_cscmr1_fixup);
	clk[emi_slow_podf]    = imx_clk_fixup_divider("emi_slow_podf", "emi_slow_sel",   base + 0x1c, 23, 3, imx6q_cscmr1_fixup);
	...
	if (clk_set_parent(clk[cko2_sel], clk[osc]))
		pr_info("Failed to set cko2 parent clk.\n");
