...
)
{
	...
	if (wrqu->data.length < (strlen(PNOSETUP_SET_CMD) + sizeof(cmd_tlv_t))) {
		WL_ERROR(("%s aggument=%d  less %d\n", __FUNCTION__, \
			(int)wrqu->data.length, (int)strlen(PNOSETUP_SET_CMD) + (int)sizeof(cmd_tlv_t)));
		goto exit_proc;
	}
		...
		char *extra)
{
	...
	WL_SOFTAP(("%s: net device:%s, buf_sz:%d\n",
		__FUNCTION__, dev->name, (int)sizeof(mac_buf)));
...
)
{
	...
	if (wrqu->data.length < (strlen(CSCAN_COMMAND) + sizeof(cscan_tlv_t))) {
		WL_ERROR(("%s aggument=%d  less %d\n", __FUNCTION__, \
			wrqu->data.length, (int)strlen(CSCAN_COMMAND) + (int)sizeof(cscan_tlv_t)));
		goto exit_proc;
	}
...
)
{
	...
	if (cmd < SIOCIWFIRST ||
		...
		!(handler = wl_iw_handler[IW_IOCTL_IDX(cmd)])) {
			printk(KERN_ERR"fxz-%s: error in cmd[%x]\n", __func__, cmd);
...
#endif 
	case SIOCSIWPRIV:
		printk(KERN_ERR"fxz-%s: call SIOCSIWPRIV\n", __func__);
		max_tokens = wrq->u.data.length;
		break;
	}
	...
	if (max_tokens && wrq->u.data.pointer) {
		if (wrq->u.data.length > max_tokens) {
			...
			ret = -E2BIG;
			printk(KERN_ERR"fxz-%s: error in cmd=%x wrq->u.data.length=%d  > max_tokens=%d\n", __FUNCTION__, cmd, wrq->u.data.length, max_tokens);
			goto wl_iw_ioctl_done;
		}
		if (!(extra = kmalloc(max_tokens * token_size, GFP_KERNEL))) {
			ret = -ENOMEM;
			printk(KERN_ERR"fxz-%s: kmalloc failed\n", __func__);
			goto wl_iw_ioctl_done;
		}

		if (copy_from_user(extra, wrq->u.data.pointer, wrq->u.data.length * token_size)) {
			kfree(extra);
			printk(KERN_ERR"fxz-%s: copy_from_user failed\n", __func__);
			ret = -EFAULT;
			goto wl_iw_ioctl_done;
		}
	}
	...
	printk(KERN_ERR"fxz-%s: handler[%d]\n", __func__, ret);
	if (extra) {
		if (copy_to_user(wrq->u.data.pointer, extra, wrq->u.data.length * token_size)) {
			kfree(extra);
			printk(KERN_ERR"fxz-%s: copy_to_user failed\n", __func__);
			ret = -EFAULT;
			goto wl_iw_ioctl_done;
		}
