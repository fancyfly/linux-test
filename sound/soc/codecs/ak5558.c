...
struct ak5558_priv {
	...
	struct gpio_desc *pdn_gpio;	/* Power on / Reset GPIO */
};
...
static int ak5558_init_reg(struct snd_soc_codec *codec)
{
	...
	if (ak5558->pdn_gpio) {
		gpiod_set_value_cansleep(ak5558->pdn_gpio, 0);
		usleep_range(1000, 2000);
		gpiod_set_value_cansleep(ak5558->pdn_gpio, 1);
		usleep_range(1000, 2000);
	}
			    ...
			    const struct i2c_device_id *id)
{
	...
	ak5558->pdn_gpio = devm_gpiod_get_optional(&i2c->dev, "asahi-kasei,pdn",
						   GPIOD_OUT_LOW);
	if (IS_ERR(ak5558->pdn_gpio)) {
		ret = PTR_ERR(ak5558->pdn_gpio);
		dev_err(&i2c->dev, "Failed to get reset line: %d\n", ret);
		return ret;
	}
