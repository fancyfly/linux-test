...
static int ov5645_try_fmt(struct v4l2_subdev *sd,
...
static int ov5645_s_fmt(struct v4l2_subdev *sd,
			struct v4l2_mbus_framefmt *mf)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ov5645 *sensor = to_ov5645(client);
	...
	if (!ov5645_find_datafmt(mf->code))
		return -EINVAL;

	ov5645_try_fmt(sd, mf);
	sensor->fmt = ov5645_find_datafmt(mf->code);

	return 0;
}

static int ov5645_g_fmt(struct v4l2_subdev *sd,
			struct v4l2_mbus_framefmt *mf)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ov5645 *sensor = to_ov5645(client);
...
static int ov5645_enum_fmt(struct v4l2_subdev *sd, unsigned int index,
			   u32 *code)
{
	if (index >= ARRAY_SIZE(ov5645_colour_fmts))
		return -EINVAL;

	*code = ov5645_colour_fmts[index].code;
	return 0;
}
