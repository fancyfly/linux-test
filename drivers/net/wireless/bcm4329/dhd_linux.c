...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	strcpy(net->name, "wlan0");/*added by fangxiaozhi to mask net name to be wlan0*/
...
dhd_module_init(void)
{
	...
	printk(KERN_ERR"fxz-%s: Invalid module\n", __func__);
		DHD_ERROR(("Invalid module parameters.\n"));
		return -EINVAL;
	} while (0);

	/* Call customer gpio to turn on power with WL_REG_ON signal */
	printk(KERN_ERR"fxz-%s: WLAN_POWER_ON\n", __func__);
	...
	if (error) {
		...
		printk(KERN_ERR"fxz-%s: wifi_add_dev err\n", __func__);
		goto fail_0;
	}

	/* Waiting callback after platform_driver_register is done or exit with error */
	if (down_timeout(&wifi_control_sem,  msecs_to_jiffies(15000)) != 0) {
		printk(KERN_ERR"fxz-%s: timeout\n", __func__);
		error = -EINVAL;
