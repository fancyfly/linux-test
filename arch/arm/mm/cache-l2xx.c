/*
 *  linux/arch/arm/mm/cache-l2xx.c
 *
...
#ifdef CONFIG_CPU_CACHE_L220
#define L2_CLEAN_LINE_DONE()	while(__raw_readl(L2_CLEAN_LINE_REG)&1)
#else
#define L2_CLEAN_LINE_DONE()
#endif

#define L2_CLEAN_INV_LINE(x)	__raw_writel(x, L2_CLEAN_INV_LINE_REG)
#ifdef CONFIG_CPU_CACHE_L220
#define L2_CLEAN_INV_LINE_DONE() while(__raw_readl(L2_CLEAN_INV_LINE_REG)&1)
#else
#define L2_CLEAN_INV_LINE_DONE()
#endif

#define L2_INV_LINE(x)		__raw_writel(x, L2_INV_LINE_REG)
#ifdef CONFIG_CPU_CACHE_L220
#define L2_INV_LINE_DONE()	while(__raw_readl(L2_INV_LINE_REG)&1)
#else
#define L2_INV_LINE_DONE()
#endif

#define L2_CLEAN_INV_WAY(x)	__raw_writel(x, L2_CLEAN_INV_WAY_REG)
#define L2_ALL_CLEANED()	while(__raw_readl(L2_CLEAN_INV_WAY_REG))
...
static int __init l2_init(void)
{
	if (!l2_policy_updated) {
		...
		if ((__raw_readl(L2_ID_REG) & 0x3F) != 0x21) {
			strcpy(l2_policy, "wb");
		}
	}
