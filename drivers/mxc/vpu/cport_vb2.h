...
#ifndef __CPORT_VB2_H__
#define __CPORT_VB2_H__
...
//
typedef struct _PORT_VB2_DATA_REQ
{
    struct list_head        list;
    struct vb2_buffer		*p_vb2_buf;
	ZV_BUFFER_DESCRIPTOR	buf_desc;
    ZOE_USER_PAGES          user_pages[ZV_MAX_BUF_PER_DESC];
	int						n_id;
} PORT_VB2_DATA_REQ, *PPORT_VB2_DATA_REQ;
...
struct c_port
{
	...
	PORT_VB2_DATA_REQ           m_port_reqs[ZV_AVLIB_MAX_DATA_ENTRIES];

    int                         m_buf_type; // v4l2_buf_type
    zoe_bool_t                  m_vb2_q_inited;
    struct vb2_queue            m_vb2_q;    // vb2 queue
	struct list_head            m_drv_q;    // driver queue
    struct semaphore            m_drv_q_lock;
    void                        *m_alloc_ctx;
};
