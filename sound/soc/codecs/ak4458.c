...
static int ak4458_set_dai_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute) { /* SMUTE: 1 , MUTE */
		...
		gpiod_set_value_cansleep(ak4458->mute_gpio, 1);
		dev_dbg(dai->dev, "%s External Mute = ON\n", __func__);
	} else { /* SMUTE: 0 ,NORMAL operation */
		gpiod_set_value_cansleep(ak4458->mute_gpio, 0);
...
static void ak4458_init_reg(struct snd_soc_codec *codec)
{
	...
	gpiod_set_value_cansleep(ak4458->mute_gpio, 1); /* External Mute ON */

	gpiod_set_value_cansleep(ak4458->pdn_gpio, 0);
	usleep_range(1000, 2000);
	gpiod_set_value_cansleep(ak4458->pdn_gpio, 1);
	usleep_range(1000, 2000);
...
int ak4458_probe(struct device *dev, struct regmap *regmap)
{
	...
	ak4458->pdn_gpio = devm_gpiod_get(dev, "asahi-kasei,pdn",
						GPIOD_OUT_LOW);
	if (IS_ERR(ak4458->pdn_gpio)) {
		ret = PTR_ERR(ak4458->pdn_gpio);
		dev_err(dev, "Failed to get reset line: %d\n", ret);
		return ret;
	}

	ak4458->mute_gpio = devm_gpiod_get(dev, "asahi-kasei,mute",
						GPIOD_OUT_LOW);
	if (IS_ERR(ak4458->mute_gpio)) {
		ret = PTR_ERR(ak4458->mute_gpio);
		dev_err(dev, "Failed to get mute line: %d\n", ret);
		return ret;
	}
