...
static int configure_aif_clock(struct snd_soc_codec *codec, int aif)
{
	...
	if (rate && rate < 3000000)
		dev_warn(codec->dev, "AIF%dCLK is %dHz,"
			"should be >=3MHz for optimal performance\n",
			aif + 1, rate);
...
SND_SOC_DAPM_SUPPLY("MICBIAS Supply", SND_SOC_NOPM, 0, 0, micbias_ev,
			    ...
			    struct snd_soc_dai *dai)
{
	...
	if (wm8994->aifclk[id] == 0) {
		dev_dbg(dai->dev, "AIF%dCLK not configured\n", dai->id);
		return -EINVAL;
	}
...
static int wm8994_probe(struct platform_device *pdev)
{
	...
	switch (control->type) {
	case WM8994:
		snd_soc_dapm_new_controls(codec, wm8994_specific_dapm_widgets,
					  ARRAY_SIZE(wm8994_specific_dapm_widgets));

		if (wm8994->revision < 4) {
			snd_soc_dapm_new_controls(
					codec, wm8994_lateclk_revd_widgets,
					ARRAY_SIZE(wm8994_lateclk_revd_widgets));
			snd_soc_dapm_new_controls(
					codec, wm8994_dac_revd_widgets,
					ARRAY_SIZE(wm8994_dac_revd_widgets));
			snd_soc_dapm_new_controls(
					codec, wm8994_adc_revd_widgets,
					ARRAY_SIZE(wm8994_adc_revd_widgets));
		} else {
			snd_soc_dapm_new_controls(
					codec, wm8994_lateclk_widgets,
					ARRAY_SIZE(wm8994_lateclk_widgets));
			snd_soc_dapm_new_controls(
					codec, wm8994_dac_widgets,
					ARRAY_SIZE(wm8994_dac_widgets));
			snd_soc_dapm_new_controls(
					codec, wm8994_adc_widgets,
	...
	case WM8994:
		if (wm8994->revision < 4) {
			snd_soc_dapm_add_routes(
					codec, wm8994_revd_intercon,
					ARRAY_SIZE(wm8994_revd_intercon));
			snd_soc_dapm_add_routes(
					codec, wm8994_lateclk_revd_intercon,
					ARRAY_SIZE(wm8994_lateclk_revd_intercon));
		} else {
			snd_soc_dapm_add_routes(
					codec, wm8994_lateclk_intercon,
