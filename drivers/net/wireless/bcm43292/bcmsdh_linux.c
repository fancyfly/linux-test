...
int bcmsdh_probe(struct device *dev)
{
	...
	printk(KERN_ERR"fxz-%s: attach failed\n", __func__);
		goto err;
	}
	if (!(sdhc = MALLOC(osh, sizeof(bcmsdh_hc_t)))) {
		SDLX_MSG(("%s: out of memory, allocated %d bytes\n",
			__FUNCTION__,
			MALLOCED(osh)));
	printk(KERN_ERR"fxz-%s: malloc failed\n", __func__);
		goto err;
	}
	...
	printk(KERN_ERR"fxz-%s: bcmsdh_attach failed\n", __func__);
		goto err;
	}
#else
	if (!(sdh = bcmsdh_attach(osh, (void *)r->start,
	                          (void **)&regs, irq))) {
		printk(KERN_ERR"fxz-%s: non mmc attach failed\n", __func__);
		SDLX_MSG(("%s: bcmsdh_attach failed\n", __FUNCTION__));
		goto err;
	}
#endif /* BCMLXSDMMC */
	...
	printk(KERN_ERR"fxz-%s: vendorid[%d]\n", __func__, vendevid);
	/* try to attach to the target device */
	if (!(sdhc->ch = drvinfo.attach((vendevid >> 16),
	                                 (vendevid & 0xFFFF), 0, 0, 0, 0,
	                                (void *)regs, NULL, sdh))) {
		printk(KERN_ERR"fxz-%s: drv attach failed\n", __func__);
