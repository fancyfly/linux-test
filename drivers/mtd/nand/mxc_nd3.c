...
{
	if (useirq) {
		if ((NFC_IPC & NFC_INT) == 0) {
			NFC_CONFIG2 &= ~NFC_INT_MSK;	/* Enable interrupt */
			wait_event(irq_waitq, NFC_IPC & NFC_INT);
			NFC_IPC &= ~NFC_INT;
		}
	} else {
		while (1) {
...
static void send_cmd(u16 cmd, bool useirq)
{
	...
	NFC_FLASH_ADDR_CMD = (u16) cmd;
	/* ack pending intr(if any), before issue of any command  */
	NFC_IPC &= ~NFC_INT;
	LAUNCH_NFC = NFC_CMD;
...
static void send_addr(u16 addr, bool useirq)
{
	...
	NFC_FLASH_ADDR_CMD = (u32) addr << NFC_FLASH_ADDR_SHIFT;
	/* ack pending intr(if any), before issue of any command  */
	NFC_IPC &= ~NFC_INT;
	LAUNCH_NFC = NFC_ADDR;
...
static void send_prog_page(u8 buf_id)
{
	...
	NFC_CONFIG1 = (NFC_CONFIG1 | NFC_SET_RBA_BITS) & NFC_SET_RBA0;
	/* ack pending intr(if any), before issue of any command  */
	NFC_IPC &= ~NFC_INT;
	LAUNCH_NFC = NFC_INPUT;
...
static u16 get_dev_status(void)
{
	...
	*(u32 *) mainBuf = 0x0;
	 ...
	 */
	NFC_CONFIG1 = (NFC_CONFIG1 | NFC_SET_RBA_BITS) & NFC_SET_RBA1;

	/* ack pending intr(if any), before issue of any command  */
	NFC_IPC &= ~NFC_INT;
	/* Read status into main buffer */
	LAUNCH_NFC = NFC_STATUS;
...
static u_char mxc_nand_read_byte(struct mtd_info *mtd)
{
	...
	/* Update saved column address by 2  */
	/* In SKYE PLUS NAND access is 16 bit */
			       ...
			       int len)
{
	...
	for (; len > 0; len -= 4) {
		if (*p++ != *mainBuf++) {
			pr_info("%s *p=0x%08x *mainbuf=0x%08x at len %d",
				__FUNCTION__, (unsigned int)*(p - 1),
				*(mainBuf - 1), len);
			return -EFAULT;
		}
	}
...
#define BAD_BLK_MARKER_464      (*((volatile u16 *)IO_ADDRESS(NFC_BASE_ADDR_AXI + 0x600 + 464)))
// read column 0-1 byte, but only 1 is used for swapped main area data
#define BAD_BLK_MARKER_SP_0     (*((volatile u16*)IO_ADDRESS(NFC_BASE_ADDR_AXI + 0x800)))
...
/* This function is called while probing NAND flash.
 * In this function skyeplus related NFC initliaziations 
 * are done.
 * @param : None
 * @return: None.
 */
static void skyeplus_nand_init(void)
{
	NFC_CONFIG2 = NFC_RST;
	NFC_CONFIG2 = (NFC_CONFIG2 | NFC_SET_PPB_BITS) & NFC_SET_PPB64;	/*setting 64 pages per block */
}
...
static int __init mxcnd_probe(struct platform_device *pdev)
{
	...
	int nr_parts = 0, n;
	u32 rounded_clk_rate;

	int err = 0;

	/*Skyeplus NFC specific INIT called */
	skyeplus_nand_init();
	...
	rounded_clk_rate = clk_round_rate(nfc_clk, 20000000);
	clk_set_rate(nfc_clk, rounded_clk_rate);

	clk_enable(nfc_clk);	/* Enabled here to satisfy following reset command to succeed */

	NFC_CONFIG2 |= NFC_INT_MSK;
	...
	NFC_WRPROT = (NFC_WRPROT | NFC_SET_BLS_BITS) & NFC_BLS_UNLOCK;

	/* Blocks to be unlocked */
	/* Start Address = 0X0, End Address   = 0xFFFF */
	NFC_WRPROT_UNLOCK_BLK_ADD0 = (0x0 | (0xFFFF << NFC_UEBA0));

	/* Unlock Block Command for given address range */
	NFC_WRPROT = (NFC_WRPROT | NFC_SET_WPC_BITS) & NFC_WPC_UNLOCK;
