...
static int mxc_dpm_set_opt(struct dpm_opt *cur, struct dpm_opt *new)
{
	...
	/* I assume that we can go to DSM/STOP/WAIT modes only with the current clock settings  */

	if (!md_new->cpu || !md_new->mode) {
#ifdef CONFIG_PM
...
#ifdef CONFIG_PM
	retval = mxc_pm_dvfs(md_new->cpu, md_new->ahb, md_new->ip);
...
static int mxc_dpm_init_opt(struct dpm_opt *opt)
{
	...
	if (core != -1 && core % AHB_FREQ != 0) {
		printk(KERN_ERR "MXC DPM Error: "
		       "ARM frequency setting %u is out of range for opt "
		       "named %s. ARM frequency should be multiple of %d.\n",
		       core, opt->name, AHB_FREQ);
		return -EINVAL;
	}
...
static int mxc_dpm_get_opt(struct dpm_opt *opt)
{
	...
	md_opt->cpu = clk_get_rate(clk);
	clk = clk_get(NULL, "ahb_clk");
	md_opt->ahb = clk_get_rate(clk);
	clk = clk_get(NULL, "ipg_clk");
	md_opt->ip = clk_get_rate(clk);
		      ...
		      int count, int *eof, void *data)
{
	...
	if (dpm_initialized) {
		...
		len += sprintf(page + len,
			       " Name           Count  DPLL  ARM   AHB   IP  MODE\n");
...
{
	printk(KERN_INFO "Freescale MXC91321 Dynamic Power Management.\n");
