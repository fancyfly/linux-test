...
static int sdma_request_channel(struct sdma_channel *sdmac)
{
	...
	sdmac->bd_iram = true;
	sdmac->bd = gen_pool_dma_alloc(sdma->iram_pool, bd_size, &sdmac->bd_phys);
	if (!sdmac->bd) {
		sdmac->bd_iram = false;
		sdmac->bd = dma_alloc_coherent(NULL, bd_size, &sdmac->bd_phys, GFP_KERNEL);
		if (!sdmac->bd) {
			ret = -ENOMEM;
			goto out;
		}
