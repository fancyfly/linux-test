...
#ifdef CONFIG_OF
static struct of_device_id mxc_mlb150_dt_ids[] = {
	{ .compatible = "fsl,imx6q-mlb", },
	{},
};
MODULE_DEVICE_TABLE(of, mxc_mlb150_dt_ids);
#else
#define mxc_mlb150_dt_ids NULL
#endif
...
static int __devinit mxc_mlb150_probe(struct platform_device *pdev)
{
	...
	drvdata = devm_kzalloc(&pdev->dev, sizeof(struct mlb_data), GFP_KERNEL);
	if (!drvdata) {
	...
	mlb_base = (u32)devm_request_and_ioremap(&pdev->dev, res);
...
#ifdef CONFIG_REGULATOR
	mlb_vcc = of_get_property(np, "vmlb-supply", NULL);
	if (mlb_vcc) {
		/* power on MLB */
		drvdata->nvcc = regulator_get(&pdev->dev, mlb_vcc);
	...
	/* enable clock */
	drvdata->clk_mlb3p = devm_clk_get(&pdev->dev, "mlb3p");
	...
	drvdata->clk_mlb6p = devm_clk_get(&pdev->dev, "mlb6p");
...
static struct platform_driver mxc_mlb150_driver = {
	.driver = {
		.name = DRIVER_NAME,
		.owner  = THIS_MODULE,
		.of_match_table = of_match_ptr(mxc_mlb150_dt_ids),
	},
