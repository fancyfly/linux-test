...
dhd_common_init(void)
{
	 ...
	 */
	//dhd_msg_level = 0xffffffff;//DHD_ERROR_VAL;//modified by fangxiaozhi
	dhd_msg_level = DHD_ERROR_VAL;//modified by fangxiaozhi
            ...
            void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_SVAL(IOV_MSGLEVEL):
		//dhd_msg_level = 0xffffffff;//int_val;
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	*/
	...
	/* Set Country code */
	strcpy((char *)dhd->dhd_cspec.ccode, "CN");
	printk(KERN_ERR"fxz-%s: ccode[%s]\n", __func__, (char *)dhd->dhd_cspec.ccode);
	if (dhd->dhd_cspec.ccode[0] != 0) {
		bcm_mkiovar("country", (char *)&dhd->dhd_cspec, \
			sizeof(wl_country_t), iovbuf, sizeof(iovbuf));
		printk(KERN_ERR"fxz-%s: iobuf[%s]\n", __func__, (char *)iovbuf);
		if ((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf))) < 0) {
			printk(KERN_ERR"fxz-%s: set country failed\n", __func__);
			DHD_ERROR(("%s: country code setting failed\n", __FUNCTION__));
		}
			printk(KERN_ERR"fxz-%s: set country successfully\n", __func__);
	}
